...
void tabulate(void)
{
    // Count the votes for each round of the election
    // if vote was eliminated, it skips to next ranked preference of that voter
...
bool print_winner(void)
{
    // Check each candidate for at least half of of votes
...
// Return true if the election is tied, false otherwise
bool is_tie(int min)
{
...
void eliminate(int min)
{
    // Eliminate all candidates that have min vote count
    for (int c = 0; c < candidate_count; c++)
    {
        if (candidates[c].eliminated == false && candidates[c].votes == min)
        {
